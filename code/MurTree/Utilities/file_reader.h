...
public:
	...
	static std::vector<FeatureVectorBinary> ReadDataDL(std::string filename, int duplicate_instances_factor = 1); 

	static std::vector<SplitData> ReadSplits(std::string data_file, std::string splits_location_prefix);
	static std::vector<FeatureVectorBinary> ReadSplits_ReadPartitionFile(std::vector<FeatureVectorBinary>& data, std::string file);
};
}